...
class Poppy(threading.Thread):
    ...
    # EX 7 - Stretching Trial
    def Stretching_1(self, counter):
        # right side
        self.poppy.l_shoulder_x.goto_position(170, 1.5, wait=False)     # raise arm up
        self.poppy.l_arm_z.goto_position(90, 1.5, wait=False)  # rotate hand
        self.poppy.l_elbow_y.goto_position(40, 1.5, wait=True)  # bend elbow
        #self.poppy.r_shoulder_x.goto_position(-30, 1.5, wait=False)     # in case of moving the other arm a little

        self.poppy.bust_x.goto_position(20, 3, wait=True)
        self.poppy.bust_x.goto_position(10, 3, wait=True)
        self.poppy.bust_x.goto_position(20, 3, wait=True)
        self.poppy.bust_x.goto_position(10, 3, wait=True)

        self.poppy.bust_x.goto_position(0, 1.5, wait=False)
        #self.poppy.r_shoulder_x.goto_position(0, 1.5, wait=False)      # in case of moving the other arm a little
        self.poppy.l_elbow_y.goto_position(90, 1.5, wait=False)
        self.poppy.l_arm_z.goto_position(0, 1.5, wait=False)
        self.poppy.l_shoulder_x.goto_position(0, 1.5, wait=True)   # put hand back down

        time.sleep(2)
        # left side
        self.poppy.r_shoulder_x.goto_position(-170, 1.5, wait=False)  # raise arm up
        self.poppy.r_arm_z.goto_position(-90, 1.5, wait=False)  # rotate hand
        self.poppy.r_elbow_y.goto_position(40, 1.5, wait=True)  # bend elbow
        # self.poppy.l_shoulder_x.goto_position(30, 1.5, wait=False)     # in case of moving the other arm a little
        ...
        self.poppy.bust_x.goto_position(0, 1.5, wait=False)
        # self.poppy.l_shoulder_x.goto_position(0, 1.5, wait=False)      # in case of moving the other arm a little
        self.poppy.r_elbow_y.goto_position(90, 1.5, wait=False)
        self.poppy.r_arm_z.goto_position(0, 1.5, wait=False)
        self.poppy.r_shoulder_x.goto_position(0, 1.5, wait=True)  # put hand back down

        if s.robot_count:
            say(str(counter + 1))
        time.sleep(1)

# EX 8 - Stretching Trial
    def Stretching_2(self, counter):
        hands_up = [self.poppy.l_shoulder_x.goto_position(170, 4, wait=False),     # raise arm up
                    self.poppy.l_arm_z.goto_position(90, 1.5, wait=False),  # rotate hand
                    self.poppy.l_elbow_y.goto_position(60, 1.5, wait=False),  # bend elbow
                    self.poppy.r_shoulder_x.goto_position(-170, 4, wait=False),
                    self.poppy.r_arm_z.goto_position(-90, 1.5, wait=False),
                    self.poppy.r_elbow_y.goto_position(60, 1.5, wait=True)]  # bend elbow

        time.sleep(2)
        hands_down = [self.poppy.l_shoulder_x.goto_position(0, 4, wait=False),     # raise arm up
                    self.poppy.l_arm_z.goto_position(0, 4, wait=False),
                    self.poppy.r_shoulder_x.goto_position(0, 4, wait=False),
                    self.poppy.r_arm_z.goto_position(0, 1.5, wait=True)]
        if s.robot_count:
            say(str(counter + 1))
        time.sleep(1)

# EX 8 - back_hand - Trial
    def back_hand(self, counter):
        hands_up = [self.poppy.l_shoulder_y.goto_position(-180, 1.5, wait=False),
                    self.poppy.l_elbow_y.goto_position(10, 1.5, wait=False),
                    self.poppy.r_shoulder_y.goto_position(-180, 1.5, wait=False),
                    self.poppy.r_elbow_y.goto_position(10, 1.5, wait=False)]
        time.sleep(2)
        hands_down = [self.poppy.l_elbow_y.goto_position(-20, 2, wait=False),
                      self.poppy.r_elbow_y.goto_position(-20, 2, wait=True)]
        if s.robot_count:
            say(str(counter + 1))
        time.sleep(1)
        # return to init position
...
if __name__ == "__main__":
    ...
    robot.exercise_demo("back_hand")
    #robot.exercise_demo("Stretching_2")
